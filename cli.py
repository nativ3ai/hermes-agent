    ...
    def run(self):
        ...
        try:
            ...
            if isinstance(_stdin_err, OSError) and getattr(_stdin_err, "errno", None) == errno.EIO:
                ...
                or (
                    isinstance(_stdin_err, OSError)
                    and getattr(_stdin_err, "errno", None) == errno.EINVAL
                )
            ):
                print(
                    f"\nError: stdin is not usable ({_stdin_err}).\n"
                    "This can happen with certain Python installations (e.g. uv-managed cPython on macOS).\n"
                    "If you're running without a TTY, use --oneshot/-z instead of interactive chat.\n"
                    "Otherwise, try reinstalling Python via pyenv or Homebrew, then re-run: hermes setup"
                )
            else:
                raise
        finally:
